...
def remove_points(to_remove, params, params_opt_exclude, variables, optimizer):
    to_keep = ~to_remove
    # print("-1、---------------------------params['semantic_ids'].shape[0]-----------------------",params['semantic_ids'].shape[0])
    # print("-1、---------------------------params['means3d'].shape[0]-----------------------",params['means3D'].shape[0])
    # print("-1、---------------------------params['rgb_colors'].shape[0]-----------------------",params['rgb_colors'].shape[0])
    keys = [k for k in params.keys() if k not in ['cam_unnorm_rots', 'cam_trans','gaussian_frame_ids']]
    
    # if 'semantic_ids' in params:
    #     params['semantic_ids'] = params['semantic_ids'][to_keep]
    ...
    for k in keys:
        ...
        if k in params_opt_exclude:
            ...
            print("-2、---------------------------params['semantic_ids'].shape[0]-----------------------",params['semantic_ids'].shape[0])
            continue
        group = [g for g in optimizer.param_groups if g['name'] == k][0]
        stored_state = optimizer.state.get(group['params'][0], None)
        if stored_state is not None:
    ...
    # print("-2、---------------------------params['means3d'].shape[0]-----------------------",params['means3D'].shape[0])
    # print("-2、---------------------------params['rgb_colors'].shape[0]-----------------------",params['rgb_colors'].shape[0])  
...
def prune_gaussians1(params,
                    ...
                    prune_dict):
    """
    扩展 SGS-SLAM 默认的 prune_gaussians，支持基于语义标签的动态物体剔除。
    
    params:            dict，包含 'means3D','log_scales','logit_opacities' 等
    params_opt_exclude: list，指定不参与优化/裁剪的参数名
    variables:         dict，额外变量。需包含 'scene_radius' 及 'semantic_labels'（可选）
    optimizer:         torch.optim.Optimizer
    iter:              当前迭代轮数
    prune_dict:        dict，剔除策略参数，新增：
                       - 'remove_semantic_dynamic' (bool)
                       - 'dynamic_label_ids' (list of int)
    """

    # 仅在 start_after ~ stop_after 之间，且每 prune_every 轮执行一次
    
    if iter <= prune_dict['stop_after'] and \
       iter >= prune_dict['start_after'] and \
       (iter % prune_dict['prune_every'] == 0):
        print("++++++++++++++++++++++++++++++++进入prune_gaussians+++++++++++++++++++++++++++++++++")
        # 选择阈值：中途用 removal_opacity_threshold，最后一次用 final_removal_opacity_threshold
        if iter == prune_dict['stop_after']:
            remove_thr = prune_dict['final_removal_opacity_threshold']
        else:
            remove_thr = prune_dict['removal_opacity_threshold']

        # 1) 透明度过低的点
        to_remove = (torch.sigmoid(params['logit_opacities']) < remove_thr).squeeze()

        # 2) 过大的点
        if iter >= prune_dict['remove_big_after']:
            scales = torch.exp(params['log_scales'])
            big_mask = scales.max(dim=1).values > 0.1 * variables['scene_radius']
            to_remove = torch.logical_or(to_remove, big_mask)

        # 3) 基于语义标签的动态物体剔除
        if prune_dict.get('remove_semantic_dynamic', False) and \
           'semantic_ids' in params:
            # params['semantic_ids'] 存的是每个高斯点的类别 ID
            # 它可能是一个形如 [N] 或 [N,1] 的 Parameter
            print("------------------------------------------------------------------")
            print("====================params['semantic_ids']======================",params['semantic_ids'])
            sem_labels = params['semantic_ids']
            print("11111111111111111111111111111111111111111111111111111111111")
            # 如果是 [N,1] 需 squeeze 成 [N]
            if sem_labels.ndim == 2 and sem_labels.shape[1] == 1:
                sem_labels = sem_labels.squeeze(1)
            sem_labels = sem_labels.long()
            print("sem_labels:",sem_labels)    
            # 构造一个和 sem_labels 在同设备的动态类别列表
            print("22222222222222222222222222222222222222222222222222222222222")
            dyn_ids = torch.tensor(prune_dict['dynamic_label_ids'],
                                   device=sem_labels.device)
            print("dyn_ids:",dyn_ids)
            # 哪些点的标签在 dyn_ids 里，就认为是动态点
            print("33333333333333333333333333333333333333333333333333333333333")
            dynamic_mask = torch.isin(sem_labels, dyn_ids)
            # 合并到 to_remove
            print("-----------------------dynamic_mask-------------------------------",dynamic_mask)
            print("4444444444444444444444444444444444444444444444444444444444")
            to_remove = torch.logical_or(to_remove, dynamic_mask)
        
    

        # 真正删除标记的点
        params, variables = remove_points(
            to_remove, params, params_opt_exclude, variables, optimizer
        )
        torch.cuda.empty_cache()
        print("===> Removing", to_remove.sum().item(), "points")
        print("Remaining Gaussians:", params['means3D'].shape[0])
 
    # 周期性重置所有高斯的不透明度（保持原逻辑）
    if iter > 0 and \
       (iter % prune_dict['reset_opacities_every'] == 0) and \
       prune_dict.get('reset_opacities', False):
        new_logit = inverse_sigmoid(
            torch.ones_like(params['logit_opacities']) * 0.01
        )
        params = update_params_and_optimizer(
            {'logit_opacities': new_logit}, params, optimizer
        )
...
def densify(params, variables, optimizer, iter, densify_dict, params_opt_exclude, device="cuda"):
    if iter <= densify_dict['stop_after']:
        ...
        if (iter >= densify_dict['start_after']) and (iter % densify_dict['densify_every'] == 0):
            ...
            # ✅ 新增 1：在 Clone 阶段剥夺动态物体的克隆权
            if densify_dict.get('remove_semantic_dynamic', False) and 'semantic_ids' in params:
                sem_labels = params['semantic_ids']
                if sem_labels.ndim == 2 and sem_labels.shape[1] == 1:
                    sem_labels = sem_labels.squeeze(1)
                dyn_ids = torch.tensor(densify_dict['dynamic_label_ids'], device=device)
                dynamic_mask = torch.isin(sem_labels.long(), dyn_ids)
                to_clone = torch.logical_and(to_clone, ~dynamic_mask) # 只有既满足条件、又不是动态物体（~mask）的才能克隆
            # ---------------------------------------------------------
            ...
            # 注意：经历过 Clone 之后，params 的数量变多了，所以要重新获取状态
            padded_grad = torch.zeros(num_pts, device=device)
            padded_grad[:grads.shape[0]] = grads
            ...
            # ✅ 新增 2：在 Split 阶段剥夺动态物体的分裂权
            if densify_dict.get('remove_semantic_dynamic', False) and 'semantic_ids' in params:
                # 重新获取更新后的标签（因为加入了新克隆的点）
                sem_labels = params['semantic_ids']
                if sem_labels.ndim == 2 and sem_labels.shape[1] == 1:
                    sem_labels = sem_labels.squeeze(1)
                dyn_ids = torch.tensor(densify_dict['dynamic_label_ids'], device=device)
                dynamic_mask = torch.isin(sem_labels.long(), dyn_ids)
                to_split = torch.logical_and(to_split, ~dynamic_mask) # 只有既满足条件、又不是动态物体的才能分裂
            # ---------------------------------------------------------
